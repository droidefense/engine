...
import com.zerjioang.apkr.analysis.dynamicscan.machine.base.struct.generic.IAtomInstance;
import com.zerjioang.apkr.analysis.dynamicscan.machine.base.struct.model.DVMInstance;
...
public class DynamicUtils {
    ...
    public static boolean verifyDexFile(byte[] dexData) {
        boolean valid = true;
        byte[] signature = ApkrConstants.DEX_FILE_MAGIC;
    ...
    public static IAtomClass[] getExecutionEntryPoints(ApkrProject currentProject) {
    ...
    private static boolean isAndroidNative(IAtomClass c) {
        ...
        boolean full = ApkrIntelligence.getInstance().isAndroidNative(clname);
        ...
        do {
            ...
            if (parent != null) {
                parentb |= ApkrIntelligence.getInstance().isAndroidNative(parent);
                clname = parent;
            }
        } while (lastPoint != -1);
        return full || parentb;
